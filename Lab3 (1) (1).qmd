...
svm_model_linear <- svm(Y ~ X1 + X2, data = training, kernel = "linear")
...
```{r}
tuned_svm <- tune(svm, Y ~ X1 + X2, data = training,
                  kernel = "linear", ranges = list(cost = 10^(-1:3)))
...
misclassification_rate <- sum(predictions != test_data$Y) / length(test_data$Y)
...
6.  The linear SVM performed better than KNN in lab 1. Given the new prediction from the Linear SVM with an accuracy of 0.7992729, I would recommend tuning the model even more by using different kernels and adjusting the cost parameter to improve the overall performance.
...
svm_results <- data.frame(
  ...
  Sensitivity = c(0.44, 0.52, 0.36, 0.7, 0.52),
  Specificity = c(0.6, 0.7, 0.84, 0.74, 0.64),  
  Misclassification = c(0.48, 0.39, 0.4, 0.30, 0.42)  
)
