...
3.  Calculating MSEs with 8 fold cross verification
    ...
    step_MSE[i]<-mean((test$mpg-step_pred)^2)
...
# Using Step Function
# Choose the best model (Step function)
best_model <- step  # Replace with natural if you choose the other

# Choose the significant value (e.g., median horsepower)
x0 <- median(Auto$horsepower)

# Define the bins using the same breaks used when fitting the model
bins_x0 <- cut(x0, breaks = 4, include.lowest = TRUE, labels = levels(bins))  # Use labels = TRUE

# Make the prediction
predicted_mpg <- predict(best_model, newdata = data.frame(bins = bins_x0))

# Print the predicted mpg at that value
print(predicted_mpg)
