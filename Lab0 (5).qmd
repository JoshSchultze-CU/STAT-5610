...
4.  Q1: A sports team wants to decide if they should trade a player based on their performance and salary. They collected data on factors such as the player's average points per game (PPG) and their annual salary. Q1 in this case X1 being the player's PPG and X2 being their salary. The class, Y, is whether or not the player should be traded. X1 represents the player's performance (PPG), and X2 is their cost (salary). This is important because it helps the team determine if a player’s performance justifies their salary and whether a trade would be beneficial for the team’s overall strategy.

```         
Q2: The model uses the collected data to predict whether a player should be traded based on their PPG and salary, looking for patterns and decision boundaries that help identify which players are likely to be traded.
