...
1.  Review and combine individual results. Which methods for clustering the data points seemed to correspond best with the pre-established groups? What $K$ gave you the best results? What linkage? What distance metric?
    ...
    We had two "best" results. One resulted from k means and the other from heiracrchal clustering. They both had similar errors (although for different groups). The means method used a k of 4. The other method that seemed to correspond best with the pre-established groups was the hierarchical clustering with a cut at K=4. The linkage was complete and the distance was euclidean. We came to this conclusion after comparing the group containment for each model printed in the individual assignments. This method successfully separated Indo students from CU students fairly well when compared to the other models. This was also verified for the attitude data, though selecting the best model here was more difficult as no model performed significantly better than the other according to our data.
...
3.  Create your team's final, best clustering of the data points. How well do the clusters correspond to the four groups?
    ...
    The clusters correspond fairly well to the four groups. IndoProf24 and StatCollab are mostly grouped together, while IndoPost24 is separated but still mixed with some other groups. SLSp25 is more scattered across different clusters, indicating some overlap. Below is the best clustering of the data points for clustering the four groups:
